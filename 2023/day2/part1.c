...
int main(void) {
  ...
  printf("\n");
  uint32_t possible_games_total = 0;

  for (uint8_t i = 0; i < game_count; i++) {
#ifdef DEBUG
    printf("Game %d: red %d, green %d, blue %d\n", games[i].id, games[i].red,
           games[i].green, games[i].blue);
#endif
    if (red < games[i].red || green < games[i].green || blue < games[i].blue) {
#ifdef DEBUG
      printf("Rejected game %d\n", games[i].id);
#endif
    } else {
#ifdef DEBUG
      printf("Accepted game %d\n", games[i].id);
#endif
      possible_games_total += games[i].id;
    }
  }
